...
export class AnalysisService {
  ...
  getPipeline() {
    return {
      ...
      storagePolicy:
        "local-first; user text and API keys stay in the deployment; async jobs are in-memory in the MVP",
    };
  }
  ...
  async analyzeBook(input: AnalyzeBookDto) {
    return this.runBookMapReduce(input);
  }
  ...
  private async runBookMapReduce(
    ...
    onPreprocess?: (preprocessing: BookPreprocessResult) => void | Promise<void>,
  ) {
    await onProgress?.({
      stage: "preprocess",
      current: 0,
    ...
    for (const chapter of chapters) {
      const chapterMap =
        input.provider.kind === "mock"
          ...
          : await this.analyzeChapterMap(input, chapter);
      chapterMaps.push(chapterMap);
      await onProgress?.({
        stage: "map",
        current: chapterMaps.length,
        total: chapters.length,
        message: `已完成 ${chapterMaps.length}/${chapters.length} 个章节 map。`,
      });
    }
    ...
    await onProgress?.({
      ...
      message: "正在把章节拆解结果 reduce 成整书资产。",
    });
    ...
    return {
      ...
      mapReduce: {
        strategy: "clean -> split chapters -> map chapter assets -> reduce book assets",
        mapCount: chapterMaps.length,
        chapterMaps,
        reducerNote:
          "整书结果由章节级摘要和资产信号汇总生成，避免一次性把超长全文塞给模型。",
      },
    };
  }

  private async analyzeChapterMap(input: AnalyzeBookDto, chapter: ChapterSegment) {
    const content = await this.modelProviders.chat(input.provider, [
      {
        role: "system",
        content:
...
`.trim();
  ...
  private bookChapterMapPrompt(input: AnalyzeBookDto, chapter: ChapterSegment): string {
    return `
...
`.trim();
  ...
  private mockChapterMap(input: AnalyzeBookDto, chapter: ChapterSegment): ChapterMapResult {
