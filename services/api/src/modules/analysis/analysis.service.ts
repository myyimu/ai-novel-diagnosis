...
export class AnalysisService {
  ...
  async listBookAnalysisJobs(limit?: number) {
    return this.persistence.listJobs(limit);
  }
  ...
  private async runBookMapReduce(
    ...
    for (const chapter of chapters) {
      ...
      await onChapterMap?.(chapterMap, [...chapterMaps], chapters.length);
      await onProgress?.({
        stage: "map",
    ...
    return {
      ...
      mapReduce: {
        strategy:
          "clean -> split chapters -> map chapter assets -> reduce book assets",
