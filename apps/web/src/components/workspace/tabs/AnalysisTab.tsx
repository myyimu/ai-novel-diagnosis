...
export function AnalysisTab({
	...
	const readinessLabel =
		researchReadiness >= 75 ? "就绪" : researchReadiness >= 50 ? "部分就绪" : "需要更多样本";
	const readinessColor =
		researchReadiness >= 75 ? "default" : researchReadiness >= 50 ? "secondary" : "outline";
		...
		<div className="space-y-4">
						...
						{hasAnalysis && (
							<div className="space-y-2 text-sm">
								...
								<div className="flex justify-between">
									...
									<span className="font-medium">
										{bookAnalysisResult.relationships?.edges?.length || 0}
									</span>
								</div>
							</div>
						)}
					...
					<CardContent className="space-y-3">
						<div className="space-y-2 text-sm">
							...
							<div className="flex justify-between">
								...
								<span className="font-medium">
									{persistedResearchLibrary?.sourceSummary?.completedBooks || 0}{" "}
									本
								</span>
							</div>
							<div className="flex justify-between">
								...
								<span className="font-medium">
									{graphNodeCount} 节点 / {graphEdgeCount} 关系
								</span>
							</div>
							<div className="flex justify-between">
							...
							<>
								...
								{persistedResearchLibrary?.graphAssets?.length > 0 && (
									<div className="space-y-2">
										...
										{persistedResearchLibrary.graphAssets
											.slice(0, 3)
											.map((asset: any) => (
												<div
													key={asset.jobId}
								...
								{persistedResearchLibrary?.comparisonSamples?.length > 0 && (
									<div className="space-y-2">
										...
										{persistedResearchLibrary.comparisonSamples
											.slice(0, 3)
											.map((sample: any) => (
												<div
													key={sample.jobId}
			...
			<Card>
				...
				<CardContent className="space-y-3">
					...
					{graphNodeCount > 0 ? (
						...
						<p className="text-sm text-muted-foreground">需要先完成全书分析</p>
					)}
				</CardContent>
			</Card>
			...
			<Card className="border-primary/20 bg-primary/5">
				<CardHeader>
					...
					<CardDescription>导出分析结果、世界书和原创化素材包</CardDescription>
				</CardHeader>
				<CardContent>
					<Button onClick={onOpenExport} className="w-full" disabled={!hasAnalysis}>
						打开导出页面
						<ArrowRight className="w-4 h-4 ml-2" />
					</Button>
				</CardContent>
