...
export function DiagnosisTab({
	...
	const _hasRubric = Boolean(rubricResult);
	const _hasScore = Boolean(scoreResult);
	...
	return (
		<div className="space-y-4">
			...
			<Card
				className={
					isPassed
						? "border-success-border bg-success-surface/10"
						: "border-warning-border bg-warning-surface/10"
				}
			>
				<CardHeader>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>上传参考章节以生成更准确的评分标准</CardDescription>
				</CardHeader>
				<CardContent className="space-y-3">
					{/* 参考章节导入（简化版） */}
					...
					<div className="flex gap-2">
						...
						{hasRubricCache && onRebuildRubric && (
							<Button variant="outline" onClick={onRebuildRubric} disabled={loading}>
								重新生成
							</Button>
						)}
					</div>
			...
			{rubricResult && (
				<Card>
					<CardHeader>
						...
						<CardDescription>基于参考章节和平台风格生成的评分指标</CardDescription>
					</CardHeader>
					...
					<CardContent className="space-y-4">
						<div className="rounded-md border border-primary/30 bg-primary/10 p-4">
							...
							<div className="mt-3 grid gap-2 text-sm">
								<p>
									<span className="text-muted-foreground">优先改：</span>
								...
								<p>
									<span className="text-muted-foreground">保留优势：</span>
									{scoreResult.strongestPoint}
								</p>
							</div>
						</div>
			...
			{(platformLabel || readingModeLabel) && (
				<Card>
					<CardHeader>
						...
						<CardDescription>当前选定的平台和策略配置</CardDescription>
					</CardHeader>
					<CardContent className="space-y-3">
						...
						{onOpenPlatformStrategy && (
							<Button
								variant="outline"
								size="sm"
								onClick={onOpenPlatformStrategy}
								className="w-full"
							>
								调整平台策略
			...
			{chapterText && (
				<Card>
					<CardHeader>
						...
						<CardDescription>当前章节文本和完成度</CardDescription>
					</CardHeader>
					<CardContent className="space-y-3">
						<div className="space-y-2 text-sm">
							...
							<div className="flex justify-between">
								...
								<span className="font-medium">
									{chapterCompletion > 0 ? `${chapterCompletion}%` : "计算中"}
								</span>
							</div>
						</div>
			...
			{chapterProjectSteps.length > 0 && (
				<Card>
					<CardHeader>
						...
						<CardDescription>当前章节在项目中的进度</CardDescription>
					</CardHeader>
					<CardContent>
						<div className="space-y-2">
							{chapterProjectSteps.map((step, index) => (
								...
								>
									<div
										className={`mt-0.5 size-5 shrink-0 rounded-full border-2 flex items-center justify-center ${
											step.done
			...
			<Card className="border-primary/20 bg-primary/5">
				<CardHeader>
					...
					<CardDescription>访问完整的章节诊断页面，包含更多详细功能</CardDescription>
				</CardHeader>
				<CardContent>
