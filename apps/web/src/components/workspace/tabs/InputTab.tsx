...
interface WorkspaceHandlers {
  ...
  onQuickReviewInputKindChange: (value: import("@/stores/workspace-store").QuickReviewInputKind) => void;
  ...
  const quickExperiencePanel = useMemo(() => {
    ...
    return (
      <QuickExperiencePanel
        ...
        onQuickReviewMustKeepMechanismsChange={handlers.onQuickReviewMustKeepMechanismsChange}
        onQuickReviewTargetReaderPleasuresChange={handlers.onQuickReviewTargetReaderPleasuresChange}
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              {handlers?.bookText || handlers?.bookFile ? (
                <span>
                  {handlers.bookTitle || "未命名书籍"} - {handlers.bookText?.length || 0} 字
                </span>
              ) : (
                "书籍上传功能将在完整实施后可用"
              )}
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              {handlers ? (
                <span>
                  当前: {handlers.providerLabel} ({handlers.providerKind === "mock" ? "本地演示" : handlers.providerModel || "预设模型"})
                </span>
              ) : (
                "AI 设置功能将在完整实施后可用"
              )}
