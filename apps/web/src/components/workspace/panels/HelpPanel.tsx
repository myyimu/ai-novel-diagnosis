...
export function HelpPanel<TView extends string = string>({ digest, onOpenView }: HelpPanelProps<TView>) {
    ...
    <div className="space-y-4">
      ...
      <Card className="border-primary/20 bg-primary/5">
        ...
        <CardContent className="space-y-2 text-sm">
          <p className="leading-5">
            <span className="font-medium">AI 时代写作瓶颈：</span>不再是"不会写"，而是"不知道写什么"。
          </p>
          <p className="leading-5">
            <span className="font-medium">诊断台方法：</span>先学会拆出爆款为什么成立，再让 AI 执行明确方向。
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {[
            { step: "1", title: "悬念/世界观型", desc: "学习制造未知和身份跃迁", view: "book" as WorkspaceView },
            { step: "2", title: "人设反差型", desc: "学习人物记忆点和关系钩子", view: "library" as WorkspaceView },
            { step: "3", title: "情绪压迫型", desc: "学习不安全感和认知冲击", view: "chapter" as WorkspaceView },
          ].map((item) => (
            <button
              key={item.step}
