...
- 一次第二意见会诊：不服判定或证据不足时，召唤立场相反的独立第二审稿人，两份判断与证据并列，分歧由你裁决。
- 对诊断报告的自由追问：回答必须引用报告内证据与原文锚点，答不了就明说。
...
```text
粘贴故事前提 -> 四层审稿 -> 不服时召唤第二审稿人会诊 -> 引导对话打磨前提 -> 再动笔
```
...
- 动笔之前先审故事前提：按故事发动机、主角欲望、持续冲突、不可替代性四层给出可回查的证据判断，结论分"值得写 / 可修 / 不值得写"，并可进入引导对话继续打磨。
- 不服判定或最薄一层证据不足时，召唤立场相反的第二审稿人会诊：一致、相邻、相反逐层比对，两份理由并列呈现，分歧不静默消解。
- 快速质检与整书体检对同一章结论矛盾时显式披露，两条报告原话并列，由作者裁决并留备注。
- 会诊、分歧与作者裁决按"作者看到的原样"记入项目病历（演示模式永不进入病历），并随项目导出包一起导出。
...
供应商返回 429 限流或瞬时 5xx 时，API 会自动退避重试（默认 2 次、2 秒起步指数退避，尊重 Retry-After；超时与鉴权错误不重试）。可用 `PROVIDER_TRANSIENT_RETRY_MAX` 和 `PROVIDER_TRANSIENT_RETRY_BASE_DELAY_MS` 调整。
...
- 诊断历史、项目导出包、真实 V1/V2 正文版本、故事体检 finding 的作者复核，以及会诊/分歧记录与作者裁决已接入后端持久化（病历只落真实模型结果，演示模式永不进入）。快速诊断的逐 issue 作者决定、实际采用项和完整独立复诊仍在补齐；在这些对象缺失时，不得把诊断历史表述为真实修改效果。真实 PostgreSQL 部署已提供 Drizzle migration，后续 schema 变更需要生成新迁移。
