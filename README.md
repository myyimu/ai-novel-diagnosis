...
本地部署的 AI 小说第一章改稿急诊工具。它不是代写工具，也不是先做一堆拆书资产，而是帮助网文新手先救自己的第一章：粘贴章节、找出最大流失点、拿到可复制给写作 AI 的改稿 Prompt；改完后再贴回来复诊，确认问题是否真的被解决。主导航只保留急诊室、深度质检和 AI 设置；整书拆解、图谱复核、历史任务、导出和样本研究都属于进阶工作流。
...
_界面仍在快速迭代中，截图可能落后于当前信息架构，请以当前版本实际页面为准。_
...
整书拆解和样本研究放在进阶区：当你已经有完整 TXT 或多个样本时，再用它们沉淀角色卡、世界书、风格规则、选题判断和可追溯证据。
...
- 改稿急诊：只粘贴第一章，也能先得到定位、卖点、最大流失点、三条改法和改稿 Prompt。
- 复测闭环：改稿前后可以对比快速点评结果，判断改动是否真的解决问题。
- 深度质检：拆解成熟参考章节，生成评分 Rubric，再按同一标准给自己的章节评分。
- 平台画像：支持目标平台、目标读者、阅读场景、分类、主题、标签和关键词。
- 数据辅助：支持展现量、点击率、阅读 30s/60s、触底率、追更率等表现归因。
- 整书拆解：进阶使用。上传 TXT，清洗文本、章节切分、异步 Map-Reduce 拆书。
- 中间结果留存：每章 map 完成后写入本地文件，token 不足或任务失败时不至于白跑。
- 关系图谱工作台：把整书拆解结果转成可点击图谱，支持总览、复核、时间线、导出、节点拖拽、图谱 JSON/SVG 导出。
- 图谱复核队列：对弱证据关系、孤立节点和疑似重复节点做确认、改标签、合并、忽略，并把人工修正写入导出 JSON。
- 整书结果管理：历史任务和导出不再作为主导航，统一收在整书拆解页内。
- 导出工具：支持 Markdown、JSON、Tavern 角色卡、World Book、SillyTavern World Info、续写包、风格圣经、卷纲、提示词包和 Do Not Copy 清单。
...
如果只是想最快体验产品，Windows 用户优先使用一键启动。面向普通用户或第一次运行，推荐直接双击：
...
- 检查 Node.js / pnpm 是否符合项目版本要求。
- 缺依赖时引导安装，`scripts/start-local.cmd -a` 会尽量自动安装。
- 自动执行缺失的 `pnpm install`。
- 复用已健康运行的本项目服务；端口被占用时会在后续端口中寻找可用端口。
- 分别打开 API / Web PowerShell 窗口，并把日志写入 `.local/run-logs`。
- 启动完成后自动打开 Web 页面。

如果你已经在仓库根目录的终端里，也可以使用等价入口：
...
进入页面后可以先不配置复杂参数，直接粘贴第一章运行“改稿急诊”。共享模型不可用或排队较久时，再到“AI 设置”切换自己的模型服务。
