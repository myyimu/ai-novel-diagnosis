...
成熟样本拆解、Rubric、深度评分、整书拆解和样本研究是进阶能力，用来支撑更严肃的学习、质检和创作决策，不作为首次使用入口。

## Workspace

- `apps/web`: Next.js 控制台，负责改稿急诊、复诊对比、深度质检、整书拆解、样本研究、AI 设置和导出入口。
...
- 整书拆解：进阶入口。上传 TXT，预览章节切分，启动 Map-Reduce 整书拆解，沉淀角色卡、世界书和写作资产。
- 样本研究：进阶入口。把已拆解样本、评分证据和多书对比压缩成可追溯的选题/改稿判断。
