...
成熟样本拆解、Rubric、深度评分、整书拆解、图谱复核和样本研究是进阶能力，用来支撑更严肃的学习、质检和创作决策，不作为首次使用入口。
...
- 整书拆解：进阶入口，通过 `/book` 进入。上传 TXT，预览章节切分，启动 Map-Reduce 整书拆解，沉淀角色卡、世界书、关系图谱和写作资产。
- 关系图谱工作台：整书拆解完成后的核心视图，支持总览、复核、时间线和图谱导出。
- 历史任务/导出工具：不再作为主导航独立页面，围绕 `/book` 的整书任务做恢复、查看和素材导出。
- 样本研究：样本足够后才有价值的进阶能力，作为整书拆解后的研究工作流，而不是默认入口。
...
- `/history`、`/export`、`/library`：重定向到 `/book`，由整书页内的工具面板或后续条件入口承接。
...
改稿急诊用于降低首次使用门槛，不要求成熟样本、Rubric 或完整平台参数。
