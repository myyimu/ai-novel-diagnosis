...
## 参考项目借鉴：Webnovel Writer

参考项目路径：

```text
D:\IdeaProjects\gitWork\webnovel-writer-master
```

该项目定位为 Claude Code 上的长篇网文写作插件，核心目标是让 AI 写到几十章、几百章后仍然保持设定、一致性、伏笔和记忆。它是“长篇写作执行系统”，而本项目的目标是“网文诊断、AI 生成稿质检与 Prompt 迭代系统”。两者方向相邻，但不应直接合并。

许可证注意：参考项目使用 GPL-3.0。本项目当前为 MIT，因此只能借鉴产品思想、信息架构和工程原则，不复制代码、模板文本或 schema 原文。
...
也就是说，我们不应把一次模型调用当成产品，而要把“生成后的判断和迭代”设计成稳定流水线。
...
- 不用总分替代问题清单。
- reviewer 输出结构化 JSON。
- blocking issue 决定是否能进入下一步。
- 分数只用于趋势观测，不替代 issue 明细。

这对本项目很重要。我们的 Quick Review v2 也应降低“分数崇拜”，把核心判断改成：
...
- `continue`: 值得继续改。
- `revise`: 有潜力，但必须先修关键问题。
- `rebuild`: 脑洞/结构需要重构。
- `discard`: 不建议继续投入。

#### 3. 写作闸门

参考项目有 prewrite / precommit / postcommit gate。

本项目可借鉴为三类质检闸门：

```text
生成前闸门：脑洞、目标读者、卖点承诺是否够清楚。
生成后闸门：AI 初稿是否兑现承诺，是否有追读力。
复诊闸门：上一轮 Prompt 是否解决原问题。
```
...
参考项目把记忆拆成剧情事实记忆和项目经验记忆，其中 `/webnovel-learn` 会把好用写法归类为 hook、pacing、dialogue、payoff、emotion 等模式。

本项目应借鉴“经验记忆”而不是“剧情事实记忆”作为第一优先级：
...
这正好对应我们要做的作者/团队方法论沉淀。

#### 5. 作者友好报告

参考项目强调最终报告不能丢原始 JSON、traceback 或长命令日志给作者，而是用：
...
本项目也应避免把复杂诊断结构直接暴露给用户。前端报告可以固定为：
...
参考项目的 Dashboard 用于查看项目状态、实体图谱、章节内容、伏笔和追读力数据。
...
#### 1. 不做 Claude Code 插件式命令系统

参考项目非常适合 Claude Code 用户，但本项目当前是 Web + API 产品。不要把 `/webnovel-*` 斜杠命令模式搬进 Web 产品。

#### 2. 不做完整长篇写作执行链

参考项目从初始化、卷纲、写章、提交、备份全链路接管写作流程。本项目不应和它抢“生成执行系统”，而应做生成后的质检、归因和迭代。

#### 3. 不优先做复杂剧情事实记忆

剧情事实记忆对长篇续写很重要，但对本项目 P0 不是最高优先级。我们应先做作者/团队方法论记忆。

#### 4. 不复制题材模板和写法库

参考项目内置大量题材模板、CSV 和写作 reference。由于许可证和产品定位差异，本项目不复制这些内容。后续如需题材库，应自行设计或使用可兼容授权的数据。
