...
状态：部分实施，P0/P1 主链路已落地并通过验证；P2/P3 待实施  
日期：2026-07-13  
...
| P2：可选 Token 消耗拆分 | 待实施 | 独立方法论卡接口、结果页主动触发、平台适配独立入口尚未完成。 |
...
未执行：显式暂存、提交和推送。原因是当前工作区存在多项既有未提交改动，不能把本次变更与其他改动混合暂存。
...
快速诊断应成为一次轻量、可信、可解释的正文急诊，而不是把平台分析、完整评分、Prompt
...
- `includeMethodologyCards` 在 P0 阶段保留；完成独立方法论接口后标记废弃，但暂不删除。
...
- `[已完成] src/modules/analysis/analysis.service.spec.ts`
  - 增加第 14 节要求的单元测试。
- `[待实施] 后续新增方法论 DTO、controller 路由和 service 方法。`
...
1. `[待实施]` 用户未点击“沉淀方法论卡”时不调用方法论接口。
2. `[待实施]` 方法论请求不包含整章正文。
3. `[待实施]` 方法论生成失败不影响诊断会话。
4. `[待实施]` 平台分析未提供必要上下文时返回校验错误，而不是猜测。
...
- 平台适配独立入口。
- 完整评分继续使用独立流程。
- 废弃快速诊断中的 `includeMethodologyCards`。
