...
> 快速诊断当前实现的修正、提示词瘦身和可执行迁移步骤，以
> [`quick-diagnosis-v3-execution-spec.md`](./quick-diagnosis-v3-execution-spec.md)
> 为准。本文保留为工作流演进记录和长期产品规划；两者冲突时，快速诊断 V3 执行规格优先。
> 当前状态：快速诊断 V3 的 P0 已完成，P1 主链路已完成并通过验证；P2/P3 仍以 V3 执行规格中的待实施项为准。
...
12. 已新增后端持久化第一版：`workspace_projects`、`revision_sessions`、`methodology_cards` 写入 PGlite/PostgreSQL，前端启动时拉取资产，诊断完成、项目创建和备注保存会同步后端。
13. 已新增后端项目 Markdown 导出接口，优先从持久化资产生成导出包，前端在接口不可用时回退到浏览器本地导出。
14. 已新增 Prompt 有效率归因第一版：基于相邻复诊的分数变化、Gate 变化、旧问题是否复发、人工备注和上一轮 Prompt，判断 `Prompt 有效`、`执行不到位`、`方向错误`、`脑洞/结构阻塞` 或 `证据不足`。
15. Dashboard 和项目 Markdown 导出已展示 Prompt 归因结论和下一步动作。
16. Prompt 归因已抽到共享 `ai-core` 引擎，前端 Dashboard 和后端导出共用同一套规则，并输出置信度、诊断理由、信号强度、校准提示和待补数据。
17. 已生成真实 PostgreSQL 初始 Drizzle migration，并新增运行时迁移脚本；容器启动前会先执行迁移，PGlite 继续作为本地开发兜底。
18. 已新增项目级 Prompt 归因校准：聚合单次归因，输出样本数、平均置信度、主导归因、校准状态、证据缺口、下一步动作和模型/编辑复核提示，Dashboard 与项目 Markdown 导出共用。
19. 已新增 `fixtures/novel-diagnosis` 示例诊断资产，先覆盖玄幻、都市和言情三类开局，并用 `ai-core` 测试校验 gate、主问题类型、证据、读者影响、改稿动作、复诊检查点和方法论卡。

仍未完成：

- Prompt 归因目前仍是规则引擎，需要用真实案例校准阈值；下一步可引入模型辅助复核，但应保留规则信号作为可审计底座。
...
方法论库第一版不做复杂账号系统，采用本地优先持久化：

- 浏览器 localStorage：作为 UI 缓存和离线兜底。
- 后端 PGlite/PostgreSQL 表：保存复诊历史、人工备注、方法论卡和项目导出资产。

当前已接入后端持久化第一版和真实 PostgreSQL 初始迁移，下一步应完善 Prompt 有效率归因的真实案例校准。
